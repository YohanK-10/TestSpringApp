package com.atlasmind.atlaswatch.support;
...
public final class TestFixtures {
    ...
    public static Keyword keyword(Long id, Integer tmdbId, String name) {
        Keyword keyword = new Keyword();
        keyword.setId(id);
        keyword.setTmdbId(tmdbId);
        keyword.setName(name);
        return keyword;
    }

    public static MovieKeyword movieKeyword(Movie movie, Keyword keyword) {
        return new MovieKeyword(movie, keyword);
    }
    ...
    public static MovieDto movieDto(long tmdbId, String title, List<Integer> genreIds) {
        return new MovieDto(tmdbId, title, title + " overview", 88.8, "/poster.jpg",
                "/backdrop.jpg", "2024-01-01", 8.5, 4_000, genreIds);
    }

    public static MovieDetailDto movieDetailDto(long tmdbId, String title, List<MovieDetailDto.Genre> genres) {
        return movieDetailDto(tmdbId, title, genres, List.of());
    }

    public static MovieDetailDto movieDetailDto(
            long tmdbId,
            String title,
            List<MovieDetailDto.Genre> genres,
            List<MovieDetailDto.Keyword> keywords
    ) {
        return new MovieDetailDto(
                ...
                genres,
                101.5,
                new MovieDetailDto.KeywordPayload(keywords, null)
        );
    }
